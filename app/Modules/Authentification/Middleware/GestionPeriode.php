        ...
        if (Auth::check()) {
            ...
            if ($entrepriseId) {
                try {
                    ...
                    $dateFinStr = $latestPeriode->date_fin instanceof Carbon 
                        ? $latestPeriode->date_fin->toDateString() 
                        : (is_string($latestPeriode->date_fin) ? $latestPeriode->date_fin : Carbon::parse($latestPeriode->date_fin)->toDateString());
                    ...
                    if ($today > $dateFinStr) {
                        ...
                        $dateDebut = Carbon::parse($dateFinStr)->addDay();
                        ...
                        session([
                            ...
                            'active_periode_debut' => $latestPeriode->date_debut instanceof Carbon ? $latestPeriode->date_debut->toDateString() : (string)$latestPeriode->date_debut,
                            'active_periode_fin'   => $latestPeriode->date_fin instanceof Carbon ? $latestPeriode->date_fin->toDateString() : (string)$latestPeriode->date_fin,
                        ]);
                    }
                    ...
                    \Illuminate\Support\Facades\Log::warning('Erreur GestionPeriode middleware: ' . $e->getMessage());
                }
            }
        }
